...
## Measured installed ISO result

The private installed-ISO milestone passed on 2026-09-04 without changing the ordinary Omarchy browser default or installing a background service or managed Chromium policy. A uniquely tagged ISO was built from Kids `91e008d`, generic ISO workflow `8cebe11`, Omarchy `5df95727`, and local package recipe `159a5ee`; the existing upstream baseline ISO remained byte-identical and separate.

The exact Kids ISO is `/home/jake/Projects/omarchy-iso/release/omarchy-2026.09.04-x86_64-kids-demo-20260904-074211-170516200.iso`, 6,209,560,576 bytes, SHA-256 `1fd7379385b64c3746a8f32e94e63226766de19dfb6347f6ec6cbd28da641368`. Its sole matching local archive is `omarchy-kids-browser-filter-demo-0.1.0-1-x86_64.pkg.tar.zst`, 23,034,986 bytes, SHA-256 `dd8b39035959d92192e3917cf3598e5b288a95415fccbe8fa5f29cab1db61749`. The build completed in 382.349 seconds.

The real graphical configurator installed that ISO into a fresh 40 GiB qcow2 base in 402.701 seconds. The installed pacman and configurator logs both identify the private package. The 6,440,747,008-byte base had SHA-256 `9ee290aa2be07b827ad4a363aaaaeb75591554139665dead9f31bb9f69a7b3bf`, remained unchanged through overlay acceptance, and passed a non-repairing `qemu-img check`. Its copied OVMF vars was owner-writable at mode `0600`. The first bounded console-bootstrap attempt timed out after 120 seconds; the second succeeded and the retry evidence is retained.

Canonical reuse-base acceptance completed in 154.126 seconds. Normal installed Omarchy acceptance passed in 82 seconds with no failed system or user units. The external Kids suite then observed the installed package and a new headed Chromium client, ran the dedicated launcher, and collected a nonempty summary, 34-line metric stream, and acceptance log. Independent checks reproduced the complete summary predicate and exact four-key metric schema.

The installed controlled fixture reported ONNX Runtime 1.27.1 and model SHA-256 `c15d8273adad2d0a92f014cc69ab2d6c311a06777a55545f2c4eb46f51911f0f`. It intercepted 17 image responses, continued 16, deterministically replaced index 5, left zero unresolved, revealed after 2 ms, and shut down cleanly. Its 1,500 ms requested/actual hold produced three cover screenshots and 1,413,000 sampled covered pixels. The reveal sample contained 16 safe colors and the magenta placeholder while excluding the original flagged color. All 17 DOM entries were present, and no disposable profile, launched Chromium process, QEMU process, or configured listener remained after the run.

The install evidence is under `omarchy-iso/test-runs/omarchy-2026.09.04-x86_64-kids-demo-20260904-074211-170516200/runs/20260904-035052`; canonical acceptance evidence is under the sibling run `20260904-040008`; complete host logs and validation records are under `.superpowers/sdd/2026-09-03-kids-iso-controlled-demo-implementation/task-5-artifacts/`. The exact identities, hashes, package manifest, individual timings, retry history, and evidence checks are recorded in [the experiment results](../docs/experiment-results.md).

This installed result proves only Arch packaging, offline installation, dedicated controlled-launcher execution, and the harmless 17-image fixture's interception, cover, deterministic replacement, privacy schema, detector identity, and cleanup. It does not show that the current executable can browse arbitrary sites; validate pornography-classifier accuracy or false-positive/recall behavior; survive adversarial or bypass attempts; provide a tamper-resistant cover, supervised service, safe default browser, or machine-wide managed policy; or satisfy model and project redistribution requirements.
